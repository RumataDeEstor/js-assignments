...
function isInsideCircle(circle, point) {
    if (Math.hypot(point.x-circle.center.x,point.y-
        circle.center.y)<circle.radius) return true;
    return false; 
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
    let opening = (isStartIncluded) ? "[" : "(",
    closing = (isEndIncluded) ? "]" : ")";
    return `${opening}${Math.min(a,b)}, ${Math.max(a,b)}${closing}`;
}
...
function getMatrixProduct(m1, m2) {
    let product = new Array(m1.length).fill(
